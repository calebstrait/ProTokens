...
[vars.trial,vars.timeReward,vars.possible,vars.leftCount,eye.fixating,...
    ...
    vars.leftBot_type,vars.rghtBot_type,vars.isPrizeTrial,vars.prizeCount,vars.ITI] = deal(0);
vars.startTime = GetSecs;
%**********
               
%***** Set up
[vars.filename, foldername] = createFile('/Data/ProTokensRevised', 'PTR', initial); %Create data file

vars.daysTrials = countDayTrials('/Data/ProTokensRevised', foldername); %Count day's cumulative trials
...
vars.reward     = [.07 .28];        %Reward sizes
...
vars.ITIrange   = [.5 1.5];         %Time: intertrial interval range (randomly generated each trial)
...
vars.ITI = (rand * range(vars.ITIrange)) + vars.ITIrange(1);
safeRoll = rand;
[m,mm] = deal([1 1;1 1]);
